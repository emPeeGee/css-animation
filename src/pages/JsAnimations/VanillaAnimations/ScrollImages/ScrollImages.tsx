...
export function ScrollImages() {
  ...
  React.useEffect(() => {
    (function () {
      ...
      function randomPositiveOrNegative(min: number, max: number) {
      ...
      let rafId: any = undefined;
      let rafActive = false;
      // Container element
      const container = document.querySelector('.ScrollImages .container')!;
      // Array with `.image` elements
      const images = Array.prototype.slice.call(document.querySelectorAll('.ScrollImages .image'));
      // Variables for storing dimmensions
      let windowWidth: number, containerHeight: number, imageHeight: number;

      // Variables for specifying transform parameters and limits
      const rotateXMaxList: number[] = [];
      const rotateYMaxList: number[] = [];
      const translateXMax = -200;
      ...
      // Here we are creating it and appending it to the `ScrollImages`
      const scrollImagesBody = document.querySelector('.ScrollImages')!;
      const fakeScroll = document.createElement('div');
      fakeScroll.className = 'fake-scroll';
      scrollImagesBody.appendChild(fakeScroll);
      ...
      function updateScroll() {
        target = document.querySelector('.ScrollImages')!.scrollTop;
        startAnimation();
      }
      ...
      function updateAnimationImages() {
        ...
        images.forEach(function (image, index: number) {
          // Calculating the `intersectionRatio`, similar to the value provided by
          // the IntersectionObserver API
          intersectionRatioIndex = windowWidth > 760 ? parseInt(String(index / 2)) : index;
      ...
      document.querySelector('.ScrollImages')!.addEventListener('resize', setupAnimation);
      // Listen for `scroll` event to update `target` scroll position
      document.querySelector('.ScrollImages')!.addEventListener('scroll', updateScroll);
